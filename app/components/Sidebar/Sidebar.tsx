  ...
  function processSelectedKey(selected: string | undefined) {
    ...
    const protodef = protos.find(protoFile => {
      const match = Object.keys(protoFile.services).find(
        service =>
          service === serviceName && fileName === protoFile.services[serviceName].proto.filePath,
      )
      ...
      <div style={styles.optionsContainer}>
        <div style={{ width: '50%' }}>
          <Tooltip title="Reload" placement="bottomLeft" align={{ offset: [-8, 0] }}>
            <Button
              type="ghost"
              style={{ height: 22, paddingRight: 5, paddingLeft: 5 }}
              onClick={onReload}
            >
          ...
          <Modal
            ...
            width={750}
            footer={[
              <Button key="back" onClick={() => setImportPathsVisible(false)}>
                Close
              </Button>,
            ]}
          >
        ...
        {protos.length > 0 && (
          <Tree.DirectoryTree
            ...
            onSelect={async selectedKeys => {
              ...
              onMethodSelected(
                profoDefinitions.methodName,
                profoDefinitions.protodef.services[profoDefinitions.serviceName],
              )
            }}
